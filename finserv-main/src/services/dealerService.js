...
export const dealerSendOtp = async (email) => {
  const response = await api.post(`/dealer/send-otp?email=${email}`);
  return response.data;
};

export const dealerVerifyOtp = async (dto) => {
  const response = await api.post("/dealer/verify-otp", dto);
  return response.data;
};

export const dealerResetPassword = async (dto) => {
  const response = await api.post("/dealer/reset-password", dto);
  return response.data;
};
