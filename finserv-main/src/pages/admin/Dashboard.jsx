...
const Dashboard = () => {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      // Auto-approve documents for users whose bank was already assigned
      await Promise.all(mappedUsers.filter(user => user.bankAssigned).map(async (user) => {
        const userDocs = allDocsMap[user.userId] || [];
        if (userDocs.length > 0) {
          await Promise.all(userDocs.map((doc) => {
          ...
          {users.map((user) => (
            ...
            >
              ...
              <td className="py-5">
                                ...
                                {user.bankAssigned ? (
                  <span className="text-[#0B2A4A] font-medium">{user.bank || "Assigned Bank"}</span>
                ) : (
...
<div className="flex items-center gap-2">
                  ...
                  {user.changed && (
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!user.bank) {
                          alert("Please select a bank before assigning.");
                          return;
                        }
                        ...
                        try {
                          ...
                          try {
                            ...
                            const userDocs = docsRes.data.data || [];
                            if (userDocs.length > 0) {
                              await Promise.all(
                                userDocs.map((doc) => {
                                  if (doc.status !== "APPROVED") {
                            ...
                            console.warn("Document approval failed after bank assignment:", docErr);
                          }

                          alert(`Loan application assigned to ${user.bank} successfully! ${approvedCount > 0 ? `Approved ${approvedCount} document(s).` : `No documents found to approve.`}`);
                          setUsers((prevUsers) => prevUsers.map((u) => u.id === user.id ? { ...u, bankAssigned: true, changed: false } : u));
                          fetchAdminData();
                        } catch (err) {
                          console.error("Failed to assign bank:", err);
                          alert("Failed to assign bank. Please try again.");
                        }
                      }}
